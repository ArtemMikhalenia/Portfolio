...
$(document).ready(function () {
	$('.header__burger').click(function (event) {
		$('.header__burger,.menu-header__menu,.text-header__language').toggleClass('active');
...
function adaptive_header(w, h) {
	var headerMenu = $('.menu-header__list');
	var headerLang = $('.text-header__language');
	if (w < 479.98) {
		...
		if (headerLang.hasClass('done')) {
			headerLang.removeClass('done').prependTo($('.text-header__language'));
		}
	}
}
